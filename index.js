const mailjet = require('node-mailjet').connect(
  '19769868bb485975960e579cf0a7dbef',
  '7d28803434badc8ee1a909b65f027e70'
);
const csv = require('csv-parser');
const fs = require('fs');

const SENDER_NAME = 'Kevin';
const SENDER_EMAIL = 'kevin.tassi.pd+1@gmail.com';
...
fs.createReadStream(
  './VFinale-Listingdeputes_Senateurs-FR-Listing-General-AN.csv'
)
  .pipe(csv())
  .on('data', (row) => {
    count++;
        ...
        Messages: [
          {
            ...
            To: [
              {
                Email: SENDER_EMAIL, // TODO: replace with real email
                Name: row.nom,
              },
            ],
            ...
            HTMLPart: getTextMessage(
              getGreetings(row.sexe, row.nom.split(' ')[1]),
              row.nom_circo.trim(),
              SENDER_NAME
            ),
            CustomID: `EmailSendFrom-${SENDER_EMAIL}-to-${getEmail(
              row.emails
            )}`,
          },
        ],
